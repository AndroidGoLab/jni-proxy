...
var alarmManagerOnAlarmListenerCmd = &cobra.Command{
	Use:   "manager-on-alarm-listener",
	Short: "ManagerOnAlarmListenerService operations",
}

var alarmManagerOnAlarmListenerOnAlarmCmd = &cobra.Command{
	Use:   "on-alarm",
	Short: "OnAlarm RPC",
	RunE: func(cmd *cobra.Command, args []string) error {
		ctx, cancel := requestContext(cmd)
		defer cancel()
		client := pb.NewManagerOnAlarmListenerServiceClient(grpcConn)
		req := &pb.OnAlarmRequest{}
		resp, err := client.OnAlarm(ctx, req)
		if err != nil {
			return err
		}
		return printProtoMessage(resp)
...
func init() {
	...
	alarmManagerOnAlarmListenerCmd.AddCommand(alarmManagerOnAlarmListenerOnAlarmCmd)
	alarmCmd.AddCommand(alarmManagerOnAlarmListenerCmd)
